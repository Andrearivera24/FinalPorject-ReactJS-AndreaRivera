...
const ItemListContainer = () => {
  ...
  const natFilter = professors.filter(
    (professor) => professor.nationality === nationality
  ); //Filtro la nacionalidad en específico con el parámetro antes usado.
  //le paso las propiedades, professors y nationality a itemList
  return (
    <div>
      {nationality ? (
        <ItemList Professors={natFilter} />
      ) : (
        <ItemList Professors={professors} />
      )}
    </div>
  );
};
